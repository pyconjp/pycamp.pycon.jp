...
#. connpassイベントページの参加枠に一般枠なら「参加者（一般 会場払い 予約者のみ登録可）」学生枠なら「参加者（学生 会場払い 予約者のみ登録可）」を作成し、参加希望者にそれらに参加登録するよう伝える。当日は現地スタッフが現金で参加費を受け取り、現地スタッフのPayPalアカントで ``pycamp@pycon.jp`` に送金する。なお、この方針は極力採用せず、前述の ``1.`` か ``2.`` を採用すること。
